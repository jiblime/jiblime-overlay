...
IUSE="
	...
	parted pcap phyp pm-utils policykit +qemu rbd sasl selinux threads +udev +vepa
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-5.9.0-Replaced-deprecated-var-run-links-to-run.patch
	"${FILESDIR}"/${PN}-5.9.0-0005-release-unused-memory-back-to-the-system.patch
)
...
src_configure() {
	...
	if use threads ; then
		myeconfargs+=( --enable-threads=posix )
	else
		myeconfargs+=( --disable-threads )
	fi
